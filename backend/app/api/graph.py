"""
Graph API routes
Uses project-context pattern with server-side persistent state
"""
...
def generate_ontology():
    ...
    try:
        ...
        logger.info(f"Text extraction complete, {len(all_text)} chars")
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph build already in progress. Add force: true to rebuild",
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for graph data processing to complete
                task_manager.update_task(
                    task_id,
                    message="Waiting for data processing to complete...",
                    progress=55
                )
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": f"Graph build task started. Query progress at /task/{task_id}"
            }
        })
