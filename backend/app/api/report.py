...
def get_agent_log(report_id: str):
    ...
    Real-time step-by-step actions during report generation, including:
    - Report start, planning start/complete
    - Each section: start, tool calls, LLM response, completion
    - Report complete or failed
    
    Query parameters:
        from_line: Start line for reading (optional, default 0, for incremental fetch)
...
def get_console_log(report_id: str):
    ...
    Real-time console output (INFO, WARNING, etc.) during report generation,
    unlike the structured JSON log from agent-log endpoint,
...
def get_graph_statistics_tool():
    """
    Graph stats tool endpoint (for debugging)
