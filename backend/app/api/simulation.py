...
Step 2: Neo4j entity reading & filtering, OASIS simulation preparation & execution (fully automated)
"""
...
# Adding this prefix prevents the Agent from calling tools and forces a text reply
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview prompt by prepending prefix to prevent tool calls
...
def get_graph_entities(graph_id: str):
    ...
    Returns only nodes matching predefined entity types (nodes with Labels other than Entity)
...
def create_simulation():
    ...
    Note: Parameters like max_rounds are intelligently generated by LLM; no need to set them manually
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project graph not built yet, call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - running: Running, so preparation was done earlier
        # - completed: Run complete, so preparation was done earlier
        # - stopped: Stopped, so preparation was done earlier
...
def prepare_simulation():
    ...
    Features:
    - Auto-detects completed preparation to avoid redundant generation
    - Returns existing results immediately if already prepared
    - Supports force regeneration (force_regenerate=true)
    
    Steps:
    1. Check for existing completed preparation
    2. Read and filter entities from Neo4j graph
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Preparation already complete, skipping",
        ...
        # This lets the frontend get the expected Agent count immediately after calling prepare
        try:
            logger.info(f"Synchronously fetching entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quick entity fetch (no edge info needed, just count)
...
def get_prepare_status():
    ...
    1. Query in-progress task by task_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "prepare_info": {...}            // detail info when preparation is complete
            }
        }
    """
    ...
    try:
        ...
        # If simulation_id provided, check for completed preparation first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task:
            # Task not found, but if simulation_id provided, check for completed preparation
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    returns the newest one if multiple exist (sorted by created_at)
...
def get_simulation_history():
    ...
    Used for homepage history display, returns simulations with project name, description, and other rich info
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (read actual user-set rounds from run_state.json)
...
def get_simulation_profiles(simulation_id: str):
    """
    Get the Agent Profile for a simulation
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get Agent Profile in real-time (for viewing progress during generation)
    ...
    - Returns extra metadata (file modification time, whether generating, etc.)
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        - time_config: Time config (duration, rounds, peak/off-peak periods)
        - agent_configs: Activity config per Agent (activity level, post frequency, stance, etc.)
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation config does not exist, call /prepare first"
            }), 404
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script (shared script in backend/scripts/)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // optional: whether to dynamically update Agent activity to Neo4j graph memory
            "force": false                         // optional: force restart (stops running simulation and clears logs)
        }

    About the force parameter:
        - When enabled, stops and clears run logs if simulation is running or completed
        - Items cleared include: run_state.json, actions.jsonl, simulation.log, etc.
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) are updated to the Neo4j graph in real-time
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"Invalid platform: {platform}, must be one of: twitter/reddit/parallel"
            }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory update requires a valid graph_id, ensure the project graph is built"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Used by frontend to display real-time activity
...
def get_agent_stats(simulation_id: str):
    ...
    Used by frontend to display Agent activity ranking, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // if not specified: interview both platforms simultaneously in dual-platform simulation
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Ensure the simulation has completed and is in command-waiting mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // if not specified: interview each Agent on both platforms simultaneously
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "prompt": "What is your overall view on this?",  // required, question for all Agents
...
def close_simulation_env():
    ...
    Sends a shutdown command to the simulation to gracefully exit command-waiting mode.
