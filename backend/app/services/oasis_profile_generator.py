...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        # Neo4j客户端用于检索丰富上下文（zep_api_key 参数保留兼容性，不再使用）
    ...
    def _search_neo4j_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if not self.graph_id:
            logger.debug(f"跳过Neo4j检索：未设置graph_id")
            return results
        ...
        try:
            ...
            logger.info(
                f"Neo4j检索完成: {entity_name}, 获取 {len(results['facts'])} 条事实, "
                f"{len(results['node_summaries'])} 个相关节点"
            )

        except Exception as e:
            logger.warning(f"Neo4j检索失败 ({entity_name}): {e}")

        return results

    # 保留原方法名作为别名，兼容 _build_entity_context 中的调用
