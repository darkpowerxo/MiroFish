...
Converts entities from the Neo4j graph into the Agent Profile format required by the OASIS simulation platform

Optimizations:
1. Call Neo4j retrieval to further enrich node information
...
class OasisProfileGenerator:
    ...
    Converts entities from the Neo4j graph into Agent Profiles required by OASIS simulation
    ...
    def _search_neo4j_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Neo4j keyword search to retrieve rich information related to an entity.
        ...
        if not self.graph_id:
            logger.debug(f"Skipping Neo4j retrieval: graph_id not set")
            return results

        try:
            query_lower = entity_name.lower()
            with self.neo4j_driver.session() as session:
                # Search for facts in related edges
                edge_result = session.run(
                    """
                    MATCH ()-[r:GRAPH_EDGE {graph_id: $graph_id}]->()
                    WHERE toLower(r.fact) CONTAINS $query OR toLower(r.name) CONTAINS $query
                ...
                # Search for summaries of related nodes
                ...
                for record in node_result:
                    ...
                    if name and name != entity_name:
                        summaries.add(f"Related entity: {name}")
            ...
            logger.info(
                f"Neo4j retrieval complete: {entity_name}, retrieved {len(results['facts'])} facts, "
    ...
    _search_zep_for_entity = _search_neo4j_for_entity
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information retrieved from Neo4j
        """
        context_parts = []
        ...
        if entity.related_nodes:
            ...
            if related_info:
                context_parts.append("### Associated Entity Info\n" + "\n".join(related_info))
        
        # 4. Use Neo4j retrieval to get richer information
        neo4j_results = self._search_zep_for_entity(entity)
        ...
        if neo4j_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Facts Retrieved from Neo4j\n" + "\n".join(f"- {f}" for f in new_facts[:15]))
        
        if neo4j_results.get("node_summaries"):
            context_parts.append("### Related Nodes Retrieved from Neo4j\n" + "\n".join(f"- {s}" for s in neo4j_results["node_summaries"][:10]))
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entity: generates a specific character setting
        - Group/institutional entity: generates a representative account setting
        """
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social media user personas. Generate detailed and realistic personas for public opinion simulation, restoring existing real-world situations to the greatest extent possible. You must return valid JSON format; all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, restoring existing real-world situations to the greatest extent possible.
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institutional/group entity, restoring existing real-world situations to the greatest extent possible.
...
5. mbti: MBTI type used to describe the account style, e.g. ISTJ represents strict conservatism
6. country: Country (e.g. "China")
...
- persona must be a continuous text description, do not use newlines
...
- Institutional account speech must match its identity positioning"""
    ...
    def set_graph_id(self, graph_id: str):
        """Set graph ID for Neo4j retrieval"""
        self.graph_id = graph_id
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID for Neo4j retrieval to obtain richer context
        ...
        # Set graph_id for Neo4j retrieval
        if graph_id:
            self.graph_id = graph_id
        
        total = len(entities)
        profiles = [None] * total  # Pre-allocate list to preserve order
        completed_count = [0]  # Use a list so it can be modified in closures
        lock = Lock()
    ...
    def save_profiles(
        ...
        """
        Save Profiles to file (choose the correct format based on platform)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses a format consistent with to_reddit_format() to ensure OASIS can read correctly.
