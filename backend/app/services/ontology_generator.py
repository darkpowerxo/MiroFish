...
- Entities will mutually influence, repost, comment, and respond to each other
- We need to simulate the reactions of all parties in opinion events and information propagation paths
...
- Representatives of specific communities (e.g., alumni associations, fan groups, rights groups, etc.)
...
A. **Fallback types (must include, placed as last 2 in list)**:
   - `Person`: Fallback type for any individual person. Used when a person does not belong to a more specific person type.
   - `Organization`: Fallback type for any organization. Used when an organization does not belong to a more specific organization type.
...
B. **Specific types (8, designed based on text content)**:
   ...
   - Example: if the text involves academic events, can have `Student`, `Professor`, `University`
   - Example: if the text involves business events, can have `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various people will appear in the text, such as "primary school teacher", "random passerby", "some netizen"
...
- Person: Any individual person (used when not belonging to the above specific types)
...
class OntologyGenerator:
    ...
    # Maximum text length to send to LLM (50000 chars)
...
4. All entity types must be real-world subjects that can post and speak, not abstract concepts
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Practical limit: max 10 custom entity types, max 10 custom edge types
        MAX_ENTITY_TYPES = 10
        MAX_EDGE_TYPES = 10
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            '# Neo4j graph database is auto-managed by MiroFish backend',
            '',
            '',
