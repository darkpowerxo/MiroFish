...
Simulation report generation using LangChain + Neo4j in ReACT mode

Features:
1. Generate reports based on simulation requirements and Neo4j graph information
2. Plan the table of contents structure first, then generate section by section
3. Each section uses multi-round ReACT thinking and reflection mode
4. Supports dialogue with users, autonomously invoking retrieval tools during conversation
"""
...
class ReportLogger:
    ...
    def log_tool_result(
        ...
        """Record tool call result (full content, untruncated)"""
        self.log(
            action="tool_result",
            stage="generating",
            section_title=section_title,
    ...
    def log_llm_response(
        ...
        """Record LLM response (full content, untruncated)"""
        self.log(
            action="llm_response",
            stage="generating",
            section_title=section_title,
    ...
    def log_section_full_complete(
        ...
        The frontend should listen to this log to determine whether a section is truly complete, and to get the full content
        """
        self.log(
...
[Returned Content]
- Relevant factual text (directly citable)
...
This is not an LLM simulation, but calls the real interview interface to get raw answers from simulation Agents.
...
[Use Cases]
- Need to understand event perspectives from different roles (What do students think? What does the media think? What does the authority say?)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert writer of "Future Prediction Reports," possessing an "omniscient perspective" of the simulated world — you can observe the behavior, speech, and interactions of every Agent in the simulation.

[Core Concept]
We have built a simulated world and injected specific "simulation requirements" as variables. The evolved results of the simulated world are predictions of what may happen in the future. What you are observing is not "experimental data," but a "rehearsal of the future."

[Your Task]
Write a "Future Prediction Report" that answers:
1. Under our specified conditions, what happened in the future?
2. How did the various Agents (groups) react and act?
...
- No subsections needed; each section is written as complete content directly
...
Please examine this future rehearsal from an "omniscient perspective":
...
[Reminder] Report section count: minimum 2, maximum 5, content must be concise and focused on core prediction findings."""
...
1. [Must Call Tools to Observe the Simulated World]
   - You are observing the future rehearsal from an "omniscient perspective"
   - All content must come from events and Agent speech/actions that occurred in the simulated world
...
2. [Must Quote Agent Original Speech and Actions]
...
3. [Language Consistency - Quoted Content Must Be Translated to the Report Language]
   ...
   - Preserve the original meaning during translation, ensuring natural and fluent expression
...
═══════════════════════════════════════════════════════════════
[⚠️ Format Standards - Extremely Important!]
═══════════════════════════════════════════════════════════════

[One Section = Minimum Content Unit]
- Each section is the minimum division unit of the report
- ❌ Prohibited: using any Markdown headings within sections (#, ##, ###, ####, etc.)
- ❌ Prohibited: adding the section heading at the beginning of the content
- ✅ Section titles are added automatically by the system; you only need to write plain body content
...
Weibo, as the first venue for public opinion, serves the core function of first-publishing information:

> "Weibo contributed 68% of the first-wave voice volume..."

**Emotion Amplification Phase**

The Douyin platform further amplifies the event's impact:
...
- panorama_search: Wide-angle panoramic search, understand the full picture of events, timeline, and evolution
...
Each response you can only do one of the following two things (not simultaneously):
...
- Prohibited: including both a tool call and Final Answer in the same response
- Prohibited: fabricating tool return results (Observation); all tool results are injected by the system
...
3. Use Markdown format (but headings are prohibited):
   ...
   - ❌ Prohibited: any heading syntax such as #, ##, ###, ####
4. [Quote Format Standards - Must Be in Its Own Paragraph]
   Quotes must be in their own paragraph, with one blank line before and after, and must not be mixed into a paragraph:
...
7. [Reiterated] Do not add any headings! Use **bold** instead of subsection titles"""
...
3. After collecting sufficient information, output Final Answer (plain text, no headings)"""
...
class ReportAgent:
    ...
    # Maximum tool call count in dialogue
    ...
    def __init__(
        ...
        Args:
            ...
            zep_tools: Graph tools service (optional)
        """
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Deep interview - call the real OASIS interview API to get answers from simulation Agents (dual platform)
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempt when format 1 did not match, to avoid mismatching JSON in body text
    ...
    def _generate_section_react(
        ...
        """
        Generate a single section content using ReACT mode
        ...
        conflict_retries = 0  # Number of consecutive conflicts where tool call and Final Answer appear simultaneously
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break loop and force finish
                break
            ...
            if has_tool_calls:
                # Tool quota exhausted → inform clearly, require output Final Answer
...
class ReportManager:
    ...
    def get_console_log_stream(cls, report_id: str) -> List[str]:
        """
        Get all console log content (retrieve all at once)
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get all Agent log content (retrieve all at once)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Because section titles are added by the system, no headings should appear in the content
        ...
        while cleaned_lines and cleaned_lines[0].strip() in ['---', '***', '___']:
            cleaned_lines.pop(0)
            # Also remove blank lines after separator lines
            while cleaned_lines and cleaned_lines[0].strip() == '':
                cleaned_lines.pop(0)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble the full report from saved section files, with heading cleanup
        """
        folder = cls._get_report_folder(report_id)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate heading (same heading content appearing within 5 consecutive lines)
                is_duplicate = False
