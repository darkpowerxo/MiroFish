...
class SimulationState:
    ...
    def to_simple_dict(self) -> Dict[str, Any]:
        """Simplified state dict (for API return)"""
        return {
            "simulation_id": self.simulation_id,
            "project_id": self.project_id,
            "graph_id": self.graph_id,
...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    "reading", 100, 
                    f"Done, total {filtered.filtered_count} entities",
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No entities found matching the conditions, please check if the graph is built correctly"
                self._save_simulation_state(state)
                return state
            ...
            # Pass graph_id to enable Neo4j search, get richer context
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # Pass graph_id for Neo4j search
            ...
            # Reddit already saved in real-time during generation, saving again here to ensure completeness
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter uses CSV format! This is the OASIS requirement
                generator.save_profiles(
                    profiles=profiles,
            ...
            if progress_callback:
                progress_callback(
                    ...
                    f"Done, total {len(profiles)} profiles",
    ...
    def get_profiles(self, simulation_id: str, platform: str = "reddit") -> List[Dict[str, Any]]:
        """Get the Agent Profile for the simulation"""
        state = self._load_simulation_state(simulation_id)
        if not state:
            raise ValueError(f"Simulation not found: {simulation_id}")
