...
Run simulations in background and record each Agent action, supporting real-time status monitoring
"""
...
class SimulationRunState:
    ...
    # Independent round counts and simulation time per platform (for dual-platform parallel display)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Independent round counts and time per platform
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (optional, to truncate long simulations)
        enable_graph_memory_update: bool = False,  # Whether to update activities to Neo4j graph
        ...
        try:
            ...
            # Set working directory to simulation directory (database files will be created here)
            # Use start_new_session=True to create a new process group, enabling termination via os.killpg
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Update independent round counts and time per platform
                                    if platform == "twitter":
                            ...
                            # If graph memory update is enabled, send activity to Neo4j
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine if a platform is enabled by checking whether the corresponding actions.jsonl file exists
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            timeout: Timeout (seconds) to wait for process exit
        """
        if IS_WINDOWS:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer record's platform field, otherwise use default platform
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clear simulation run logs (for forcing a restart of the simulation)
        ...
        Note: Does not delete config file (simulation_config.json) or profile files
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts to ensure all simulation processes are cleaned up on server shutdown
        """
        ...
        # In Flask debug mode, only register cleanup in the reloader subprocess (process that actually runs the app)
        # WERKZEUG_RUN_MAIN=true indicates it is the reloader subprocess
        # If not in debug mode, this env var does not exist but cleanup still needs to be registered
        ...
        try:
            # SIGTERM: default signal of kill command
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True means environment is alive, False means it has closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform simulation, interview both platforms and return merged result
    ...
    def interview_all_agents(
        ...
        Uses the same question to interview all Agents in simulation
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation for graceful exit from waiting-for-command mode
