...
Wraps graph search, node retrieval, edge query tools for Report Agent use
(Using Neo4j instead of Zep Cloud)
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out junk content with question numbers (Question 1-9)
                skip = False
                for d in '123456789':
...
class ZepToolsService:
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        # api_key parameter retained for backward compatibility; actually uses Neo4j config
        self.driver: Driver = get_driver()
        # LLM client used by InsightForge to generate sub-questions
        self._llm_client = llm_client
        logger.info("ZepToolsService initialization complete (Neo4j backend)")
    ...
    def search_graph(
        ...
        """
        Graph keyword search (using Neo4j Cypher query)
    ...
    def _local_search(
        ...
        """
        Local keyword matching search (Neo4j implementation)

        Get all edges/nodes/activity records, perform local keyword matching
        ...
        try:
            if scope in ["edges", "both"]:
                ...
                # Search activity records (ActivityNode)
                try:
                    with self.driver.session() as session:
                        result = session.run(
                            "MATCH (a:ActivityNode {graph_id: $graph_id}) RETURN a.description AS desc LIMIT 200",
                            graph_id=graph_id,
                        )
                        for record in result:
                            desc = record.get("desc", "")
                            if desc and match_score(desc) > 0:
                                facts.append(desc)
                except Exception as ae:
                    logger.debug(f"ActivityNode search failed: {ae}")
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            with self.driver.session() as session:
                result = session.run(
                    "MATCH (n:GraphNode {uuid: $uuid}) RETURN n LIMIT 1",
                    uuid=node_uuid,
                )
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Get all graph edges, then filter those related to the specified node
    ...
    def get_simulation_context(
        ...
        Args:
            ...
            limit: Count limit per information type
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval function, auto-decomposes questions and retrieves in multiple dimensions:
...
Return a JSON-formatted list of sub-questions."""
    ...
    def panorama_search(
        ...
        Get the full picture view, including all relevant content and historical/expired information:
        1. Get all relevant nodes
        2. Get all edges (including expired/invalid ones)
        3. Categorize currently active and historical information
    ...
    def quick_search(
        ...
        1. Directly call Neo4j graph search
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Based on your character profile, all past memories and actions, "
            ...
            "4. Answer each question by number, each answer beginning with 'Question X:' (X is the question number)\n"
        ...
        # Step 4: Call the real interview API (no platform specified, both platforms by default)
        try:
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable interview subjects from the list of simulated Agents based on interview requirements.

Selection criteria:
1. Agent's identity/profession is relevant to the interview topic
2. Agent may hold unique or valuable opinions
3. Choose diverse perspectives (e.g., supporters, opponents, neutral parties, professionals, etc.)
...
- Use Chinese quotes 「」 when quoting interviewees
