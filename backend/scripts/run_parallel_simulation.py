...
- Supports single-agent and batch interviewing
...
# This fixes the issue of OASIS third-party libraries not specifying encoding when reading files
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout/stderr to UTF-8 (fixes garbled console output)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    
    # Force default encoding (affects the default encoding of the open() function)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes third-party libraries (e.g. OASIS) that read files without specifying encoding.
        """
        # Only set default encoding for text mode (non-binary) calls that do not specify encoding
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (recording each agent observation and action); we use our own action_logger.
    """
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            A dict containing results, or a dict containing an error.
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/not specified: Interview on both platforms simultaneously and return combined results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Agents that need to be interviewed on both platforms
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Build a mapping of agent_id -> entity_name from the simulation_config.
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid from the last read (using rowid instead of created_at because created_at formats differ across platforms)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Create comment: enrich with the post being commented on
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Choose which LLM to use based on arguments and config
...
async def run_reddit_simulation(
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed rowid (using rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        # Always log round start, even if no agents are active
        if action_logger:
            action_logger.log_round_start(round_num + 1, simulated_hour)
        
        if not active_agents:
            # Also log round end when no agents are active (actions_count=0)
            if action_logger:
                action_logger.log_round_end(round_num + 1, 0)
            continue
        ...
        # Fetch actual actions from DB and log them
        actual_actions, last_rowid = fetch_new_actions_from_db(
            db_path, last_rowid, agent_names
        )
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: the program does not exit after simulation completes;
    it waits for interview commands. On receiving a termination signal:
    1. Notify the asyncio loop to stop waiting.
    2. Allow the program to clean up resources (close databases, environments, etc.).
    3. Then exit.
    """
    def signal_handler(signum, frame):
        ...
        # Do not call sys.exit() directly; let the asyncio loop exit cleanly and clean up resources
