    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still processing — consider refreshing the graph manually later</span>
...
// Watch isSimulating for simulation end detection
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating — show end hint
...
const renderGraph = () => {
  ...
  // Process edge data, count edges between same node pairs and track their index
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Curvature range scales with edge count
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to pull isolated node clusters to center
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw arc departing from node and returning
      const loopRadius = 30
      // Depart from right side of node and loop back
  ...
  // Link label backgrounds (white background for readability)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect whether real drag started (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation on first detected real drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let simulation cool down if real drag occurred
        if (d._isDragging) {
