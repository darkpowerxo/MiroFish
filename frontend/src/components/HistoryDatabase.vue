      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- If more files exist, show indicator -->
...
const containerStyle = computed(() => {
  ...
  // Compute actual height: rows * card height + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    const x = offset * 35
    // Adjust starting position, close to title with appropriate spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Shorter delay expanding (50ms), longer collapsing (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs to execute (may be overridden by later scroll)
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before executing to avoid too-fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
.grid-pattern {
  ...
  /* Position from top-left; when height changes only extends at bottom, preserving existing grid position */
