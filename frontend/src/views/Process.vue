          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Awaiting ontology generation completion...</div>
              </div>
...
const goToNextStep = () => {
  ...
  alert('Environment Setup under development...')
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Re-load project data to get graph_id
        const projectResponse = await getProject(currentProjectId.value)
...
const renderGraph = () => {
  ...
  simulation.on('tick', () => {
    // Update positions of all edges (including visible line and transparent click area)
